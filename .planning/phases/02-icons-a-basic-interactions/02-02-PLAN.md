...
<task type="auto">
  ...
  <action>
Create button component tokens in W3C DTCG format ($value, $type syntax) with three-tier architecture:
...
- All descriptions in German
  ...
  <done>Button tokens define 3 variants x 3 sizes x 5 states with icon button dimensions and WCAG-compliant focus indicators</done>
</task>
...
- States table with visual descriptions and token names:
  ...
  |-------|-------------|------|--------|
  | Default | hydrophon.blau.500 | Weiß | transparent |
  | Hover | hydrophon.blau.600 | Weiß | transparent |
  | Active | hydrophon.blau.700 | Weiß | transparent |
  | Focus | hydrophon.blau.500 | Weiß | 2px blau.300 ring |
  | Disabled | neutral.200 | neutral.400 | transparent |

Secondary Button:
- Visual: Transparent background, brand color border and text (outline style)
- Purpose: Alternative actions (Abbrechen, Zurück, Mehr erfahren)
...
<!-- Icon Only -->
<button class="button button--secondary button--icon-only" aria-label="Schließen">
...
- Nicht: Buttons ohne erkennbare Grenzen (außer Tertiary)
- Nicht: Fokusindikatoren entfernen
- Nicht: Zu kleine Touch-Targets auf Mobilgeräten
- Nicht: Animierte Größenänderungen bei Hover/Active (color only per CONTEXT.md)
...
- No new errors beyond expected typography composite type warnings
  ...
  <done>Style Dictionary build includes button tokens in CSS custom properties and JSON output with all variants and sizes</done>
</task>
